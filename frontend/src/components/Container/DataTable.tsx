...
const Table: React.FunctionComponent<ITableProps> = (props) => {
  ...
  const renderDataTable = () => {
    return data.map((item) => {
      return (
        <tr
          key={item._id}
          className="border-b text-white font-semibold bg-gray-600 hover:bg-gray-500 "
        >
          ...
          <td className="px-6 py-3 ">
            <Link
              to={`${backendUrl}/shortUrl/${item.shortUrl}`}
              target="_blank"
            >
              {item.shortUrl}
            </Link>
          ...
          <td className="px-6 py-3 flex gap-2">
            <button
              ...
              className="bg-blue-300 px-2 py-1 text-gray-800 rounded-xl hover:bg-blue-400 hover:text-gray-900"
            >
              Copy
            </button>
            <button
  ...
  const copyUrl = async (url: string) => {
    await navigator.clipboard.writeText(`${backendUrl}/shortUrl/${url}`);
    toast(`URL Copied : ${backendUrl}/shortUrl/${url}`, {
      position: "top-right",
      autoClose: 5000,
  ...
  const deleteUrl = async (id: String) => {
    await axios.delete(`${backendUrl}/shortUrl/${id}`);
    updateReloadState();
  ...
  return (
    <div className="container mx-auto pt-2.5 pb-10 mb-16">
